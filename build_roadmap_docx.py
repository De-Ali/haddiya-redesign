...
add_para(
    'Technically, Haddiya is delivered as a cross-platform Flutter application (a single Dart codebase for '
    'both iOS and Android) backed by a Laravel (PHP) service layer that integrates with the ZID commerce '
    'platform as the storefront and order engine. The Laravel layer owns session and cookie handling, the '
    'ZID WebView bridge, the cart and checkout flow, guest checkout, and API ownership between the Flutter '
    'client and ZID.'
)
...
add_table(
    ['Layer', 'Components'],
    [
        ['Consumer Channels', 'Flutter app (iOS + Android), Mobile Web (PWA), Desktop Web'],
        ['Backend (Laravel)', 'Session/cookie handling, ZID API ownership, cart + checkout flow, guest flow'],
        ['Commerce Platform (ZID)', 'Storefront, catalog, orders, payments — headless APIs + WebView'],
...
doc.add_heading('2.1 Current Backend Build Plan (Flutter × Laravel × ZID)', level=2)
add_para(
...
add_table(
    ...
    [
        ...
        ['Mobile Engineers', '2', 'Full-time', 'Phases 3–7', 'Flutter (Dart); React Native-capable'],
        ['Backend Engineers', '3', 'Full-time', 'Phases 3–7', 'Laravel (PHP), ZID integration; Node/Go optional'],
...
modules = [
    ...
    ['21', 'Mobile Apps (iOS + Android)', 'Flutter single codebase (React Native-capable), ZID WebView bridge, deep-linking, biometrics, push, offline cart.', 'High', 'All consumer APIs', '24', '2 Mobile'],
...
phases = [
    ...
    ('Phase 4 — Frontend & Mobile Development · Weeks 8–22', [
        ...
        'Mobile app — Flutter (single Dart codebase), with React Native as an alternative; native modules for biometrics + payments.',
...
arch = [
    ['Mobile (Primary)', 'Flutter (Dart) — single codebase iOS + Android; React Native viable alternative', 'One team, native performance; ~80% code reuse'],
    ['Web (Frontend)', 'Next.js 14 (App Router) + TypeScript + Tailwind v4 + Framer Motion', 'SSR/ISR for SEO, mature i18n + RTL'],
    ['Backend', 'Laravel (PHP 8.3) REST APIs; Node.js (NestJS) / Go viable for hot paths', 'Owns session/cookie + ZID integration; type-safe options'],
    ['Commerce Platform', 'ZID — headless storefront + WebView', 'GCC-native catalog, orders, payments'],
    ['Session / Cookie Bridge', 'Laravel session forwarding + cookie parsing', 'Seamless auth across ZID WebView ↔ Flutter client'],
    ['API Gateway / BFF', 'AWS API Gateway + REST; GraphQL BFF or tRPC optional', 'Tailors payloads per channel'],
    ...
    ['Error Tracking', 'Sentry (Flutter + RN + Laravel + web)', 'First-class Flutter, React Native, Next.js & Laravel integration'],
...
add_table(
    ...
    [
        ...
        ['Mobile Engineers (Flutter / RN)', '2', 'Full-time', '7 months'],
...
add_bullets([
    ...
    ('Adopt a modular monolith first ', '(Laravel or NestJS domain modules), extract services only when scaling forces it.'),
    ('Use Flutter (single Dart codebase) ', 'or React Native instead of dual native apps — one team ships iOS + Android, saves ~3 months and 1.5 FTE.'),
